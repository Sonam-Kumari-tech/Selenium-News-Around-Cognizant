...
import java.util.List;
	...
	{
		ExcelUtility.path = System.getProperty("user.dir")+"/TestData/Details (1).xlsx";
	...
	row1.createCell(1).setCellValue("News Details");		
	for(int r=1;r<6;r++) { 
	...
	for(int r=9;r<14;r++) {
		XSSFRow row = ExcelUtility.s.createRow(r);
		row.createCell(1).setCellValue(firstNews.get(r-1));
	}
	...
	for(int r=16;r<21;r++) {
		XSSFRow row = ExcelUtility.s.createRow(r);
		row.createCell(1).setCellValue(secondNews.get(r-1));
	}
